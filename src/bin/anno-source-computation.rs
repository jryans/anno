...
fn collect_lines(source_file_path: &PathBuf) -> Result<HashSet<usize>> {
    ...
    let computation_report_path = preprocessed_file_path.with_extension("i.dbgcov");
    let regions_with_computation =
        fs::read_to_string(&computation_report_path).with_context(|| {
            format!(
                "Unable to read `dbgcov` report ({})",
                computation_report_path.display()
            )
        })?;

    // Collect set of lines with computation
    let mut lines_with_computation = HashSet::new();
    for regions_with_computation_line in regions_with_computation.lines() {
        // Line format:
        ...
        debug!("Matching line: {}", regions_with_computation_line);
